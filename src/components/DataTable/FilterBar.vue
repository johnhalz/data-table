...
const mergedFilters = computed(() => {
    const parent = props.columnFilters.map((f, i) => ({
        ...f,
        table: "parent",
        sourceIndex: i,
    }));
    const sub = props.subTableColumnFilters.map((f, i) => ({
        ...f,
        table: "sub",
        sourceIndex: i,
    }));
...
const showSubFilterBadge = computed(() =>
    mergedFilters.value.some((f) => f.table === "sub"),
);
...
const pendingChipRef = ref(null);
const dropdownPos = ref({ top: 0, left: 0 });
const valueInputRefs = ref({});

function updateDropdownPos() {
    // For the operator picker, anchor the dropdown's leading edge to the end of
    // the pending column chip (where the cursor would be if the user had just
    // typed the column name). For the column picker, fall back to the search input.
    if (dropdownState.value === "operators" && pendingChipRef.value) {
        const rect = pendingChipRef.value.getBoundingClientRect();
...
const placeholder = computed(() => {
    const colNames = props.allColumns
        .map((c) => c.id)
        .slice(0, 3)
        .join(", ");
...
function removeFilter(mergedIndex) {
    ...
    if (filter.table === "sub") {
        const newFilters = props.subTableColumnFilters.filter(
            (_, i) => i !== filter.sourceIndex,
        );
        emit("update:sub-table-column-filters", newFilters);
    } else {
        const newFilters = props.columnFilters.filter(
            (_, i) => i !== filter.sourceIndex,
        );
        emit("update:column-filters", newFilters);
    }
    ...
    <div class="flex items-center gap-1.5 flex-wrap flex-1 min-w-0">
        ...
        >
            ...
            <span
                ...
                >{{ getFilterOperator(filter) }}</span
            >
            <input
                :ref="
                    (el) => {
                        if (el) valueInputRefs[i] = el;
                    }
                "
                ...
                :style="{
                    color: 'var(--st-text)',
                    width:
                        Math.max(6, (getFilterValue(filter) || '').length + 1) +
                        'ch',
                }"
                placeholder="value"
        ...
        >
            <span
                v-if="
                    showSubFilterBadge ||
                    (pendingColumn && pendingColumn.table === 'sub')
                "
                class="text-[10px] font-medium uppercase px-1 rounded"
                :style="{
                    color: 'var(--st-text-placeholder)',
        ...
        <div ref="anchorRef" class="relative flex-1 min-w-[200px]">
            <input
                ...
                :style="{
                    color: 'var(--st-text)',
                    caretColor:
                        dropdownState === 'operators'
                            ? 'transparent'
                            : undefined,
                }"
                :placeholder="
                    dropdownState === 'operators'
                        ? 'Pick a filter method...'
            ...
            <Teleport to="body">
                ...
                >
                    ...
                    <div
                        v-if="filteredParentColumns.length === 0"
                ...
                >
                    <template v-for="(item, idx) in flatOperators" :key="idx">
                        ...
                        <div
                            ...
                            class="flex items-center justify-between px-3 py-1.5 cursor-pointer text-[13px]"
                            :data-highlighted="
                                selectableOperators.indexOf(item) ===
                                highlightIndex
                            "
                            :style="
                                selectableOperators.indexOf(item) ===
