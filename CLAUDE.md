...
| `contextMenuActions` | `Array` | `[]` | Extra right-click menu entries; also listed in the selection toolbar **Actions** menu when multi-selecting. Items: `{ key, label, icon?, danger?, variant?, divider?, disabled? }`. `divider: true` renders a separator (omit `key`/`label`). `disabled` may be a boolean or `(rowData) => boolean` for per-row disable on right-click; function form is ignored for bulk (items stay enabled). |
